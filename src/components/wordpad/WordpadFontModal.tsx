...
function buildFamilies(): Family[] {
    ...
    for (const item of allFontItems) {
        ...
        if (style === 'Regular')
            iconMap.set(family, item.fontUrl.endsWith('.otf') ? OpenTypeIcon : TrueTypeIcon);
    }
...
const WordpadFontModal = ({ current, onApply, onClose, style }: WordpadFontModalProps) => {
    ...
    // ── State ──────────────────────────────────────────────────────────────────
    const initFamily = FAMILIES.find(f => f.name === current.family)
        ?? FAMILIES.find(f => f.variants.some(v => v.fontUrl === current.fontUrl))
        ?? FAMILIES[0];
    ...
    // Position of portalled dropdowns (fixed to viewport because the dialog is itself positioned)
    ...
    // Position color dropdown and close on outside click
    useEffect(() => {
        if (!colorOpen) return;
    ...
    // Position script dropdown and close on outside click
    useEffect(() => {
    ...
    // Load font face when the variant changes so the Sample preview renders correctly
    ...
    return (
        <div
            className='app-window fm-window wordpad-window'
            ref={dialogRef}
        ...
        >
            ...
            <div className='fm-body'>
                        ...
                        <div className='fm-color-select' ref={colorRef}>
                            ...
                            {colorOpen && createPortal(
                                <ul
                                    ...
                                    style={{ position: 'fixed', top: listPos.top, left: listPos.left, minWidth: listPos.width, zIndex: 9999 }}
                                >
                                    {COLORS.map(c => (
                                        <li
                                            key={c.name}
                            ...
                            <div className='fm-color-select' ref={scriptRef}>
                                ...
                                {scriptOpen && createPortal(
                                    <ul
                                        ref={scriptListRef}
                                        className='fm-color-list'
                                        style={{ position: 'fixed', top: scriptPos.top, left: scriptPos.left, minWidth: scriptPos.width, zIndex: 9999 }}
                                    >
                ...
                {/* Font type notice at the bottom */}
